...
while True:
    _,frame=cap.read()
    count = {
             'a': len(os.listdir(directory+"/Hellow")),
             'b': len(os.listdir(directory+"/How are you")),
             'c': len(os.listdir(directory+"/Name")),
             'd': len(os.listdir(directory+"/Nice to meet You")),
             'e': len(os.listdir(directory+"/ThankYou")),
             }
    ...
    if interrupt & 0xFF == ord('a'):
        cv2.imwrite(directory+'Hellow/'+str(count['a'])+'.png',frame)
    if interrupt & 0xFF == ord('b'):
        cv2.imwrite(directory+'How are you/'+str(count['b'])+'.png',frame)
    if interrupt & 0xFF == ord('c'):
        cv2.imwrite(directory+'Name/'+str(count['c'])+'.png',frame)
    if interrupt & 0xFF == ord('d'):
        cv2.imwrite(directory+'Nice to meet You/'+str(count['d'])+'.png',frame)
    if interrupt & 0xFF == ord('e'):
        cv2.imwrite(directory+'ThankYou/'+str(count['e'])+'.png',frame)
